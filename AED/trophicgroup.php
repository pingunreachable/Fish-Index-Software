...
// Title of the page 
echo "<h1>Trophic Group</h1>";

//if action is blank or the add button has been selected do the code with the semi-colons
if ($_POST['action'] == "" OR $_POST['action'] == "Add Trophic")
{

//Inserting information table
echo "

<form action='trophicgroup.php' method='post'>

<p>To add a new Trophic, fill in the form and press the add Trophic button.</p>
...
<input type='submit' name='action' value='Add Trophic'> 
</form> 
<br>
...
if ($_POST['action'] == "Add Trophic")
{
	$result = "INSERT INTO trophicgroup  (group_code, group_name) VALUES('" . $_POST['group_code'] . "', '" . $_POST['group_name'] . "');"; 
if (!$result) {
    die('Invalid query: ' . mysql_error());
}
echo "<br>";
//Prints to the screen that the record has been added
echo "<h1>Trophic Added</h1>";
echo "<br>";
...
if ($_POST['action'] == "Update Trophic" OR $_POST['action'] == "Delete Trophic" OR $_POST['action'] == "Select Trophic")
{
...
if ($_POST['action'] == "Update Trophic")
{
	$result = "UPDATE trophicgroup SET group_name='" . $_POST['group_name'] . "' WHERE group_code='" . $_POST['group_code'] . "'"; 
	mysqli_query($link,$result);
}
...
if ($_POST['action'] == "Delete Trophic")
{
	$result = "DELETE FROM trophicgroup WHERE group_code='" . $_POST['group_code'] . "'";
	mysqli_query($link,$result);
}
}
...
<p>To edit or delete a Trophic, select a Trophic from the list and press the select Trophic button.</p>
...
<td>
<input type='submit' name='action' value='Select Trophic'> 
</td>
 </tr>
</table>
...
if ($_POST['action'] == 'Back to Add Trophic')
{
$_POST = array();
header('Location:trophicgroup.php'); 
}
?>
...
{
echo "<br>";
echo "<h1>Trophic Deleted</h1>";
echo "<br>";
}
}

$result = mysqli_query($link,"SELECT * FROM trophicgroup WHERE group_code='" . $_POST['group_code'] . "'");
...
<p>To edit or delete a Trophic, change the data and press the update Trophic button or to delete press the delete Trophic button.</p>
...
echo "
<input type='submit' name='action' value='Update Trophic'>
<input type='submit' name='action' value='Delete Trophic'> 
</form> 

<br>";
//the back button
echo "
<a href='trophicgroup.php'><button>Back to Add Trophic </button></a>
...
<br>
<!-- Back button -->
<form><input type='button' name='Back' value='Finished with Trophic' onClick="location.href='../samples.php'"></form>
