...
<!-- Title of the page -->
<center><h1>Search Nearshore Final Scores</h1></center>

<!-- Search box -->
<form action="SearchNSFinalScores.php" method="post">
...
<?php
	$result = mysqli_query($link,"SELECT * FROM NSFinalScores WHERE 
...
"); 
	...
	while($row = mysqli_fetch_array($result)) {
	  ...
	  echo "<td>" . setDecimal($row['No_benthic'], 2) . "</td>";
	  echo "<td>" . setDecimal($row['Prop_est_spawn'], 2) . "</td>";
	  echo "<td>" . setDecimal($row['No_est_spawn'], 2) . "</td>";
	  echo "<td>" . setDecimal($row['Prop_P_olorum'], 2) . "</td>";
	  echo "<td>" . setDecimal($row['Tot_no_P_olorum'], 2) . "</td>";
